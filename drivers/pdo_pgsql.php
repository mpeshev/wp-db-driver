...
class wpdb_driver_pdo_pgsql extends wpdb_driver {
	 ...
	 * @return bool
	 */
	public function connect( $host, $user, $pass, $port = 5432, $options = array() ) {
	    if ( $port == 3306 ) {
			$port = 5432;
	    }
		...
		if( '.sock' === substr( $port, -5 ) ) {
			...
			//$dsn = sprintf( 'export PGPASSWORD=\'%4$s\';psql --host=%1$s --port=%2$d --username=%3$s --dbname=pgwp',
			 //        $host, $port, $user, $pass );
			//  $dsn = sprintf( "pgsql:dbname=pgwp;host=$host;port=$port", $user, $pass );
			$dsn = "pgsql:dbname=pgwp;host=$host;port=$port";
		}
